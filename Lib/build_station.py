...
def main():
    ...
    parser.add_argument(
        "--jump_distance", help="Distance from which super jump starts."
    )
    parser.add_argument("--station_global_x", help="Global station position.", default="0")
    parser.add_argument("--station_global_y", help="Global station position.", default="0")
    ...
    with open(args.json_input) as json_file, open(args.cpp_output, "w") as cpp_file:
        ...
        station_local_x = float(json_input["stations"][0]["x"])
        station_local_y = float(json_input["stations"][0]["y"])
        ...
        cpp_file.write(
            "    .pos{{{:.1f}f, {:.1f}f}},\n".format(
                float(args.station_global_x), float(args.station_global_y)
            )
        )
        cpp_file.write('    .name{{"{}"}},\n'.format(args.name))
        cpp_file.write('    .desc{{"{}"}},\n'.format(args.desc.replace("|", "\\n")))
        ...
        for asteroid in json_input["asteroids"]:
            ...
            cpp_file.write(
                "        .pos{{{:.1f}f, {:.1f}f}},\n".format(
                    float(asteroid["x"]) - station_local_x,
                    float(asteroid["y"] - station_local_y),
                )
            )
